...
3.2. **Declare the run mode** in `_state.json` (atomically, tmp → rename) BEFORE any lens runs, so the `pre-lens-order-check` hook knows what it is enforcing:
   - full round → `"round_mode": {"mode": "full", "round": "<the round being run>"}`
   - single named lens → `"round_mode": {"mode": "single", "lens": "<name>", "round": "<the round being run>"}`

   The hook enforces sequential order for a full round and steps aside for the declared single lens — but only for **that** lens in **that** round, so a marker left behind by an interrupted run authorises nothing. Never hand-write a `single` marker to get past the order rule: the rule is a correctness invariant, and the escape exists for the documented `/round <lens>` flow (re-running one lens after an `/answer`, for instance), not to skip lenses inside a round.
...
5. After all lenses (or the single lens) finish:
   a. Persist `_state.json.round` = the round just run (atomically, tmp → rename). It therefore always holds the most recent completed round = the engagement's current round. Clear `round_mode` in the same write — the marker describes a run in progress, not a finished one.
